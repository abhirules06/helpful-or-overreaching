"""
resume_gemini2.py — Fill missing Gemini Study 2 transcripts only.
Counts existing transcripts per (task, condition) and runs only what's needed.
"""
...
# Count existing gemini transcripts
counts = {}
for p in glob.glob(os.path.join(TRANSCRIPT_DIR, "*.json")):
    with open(p) as f:
        try: t = json.load(f)
        except: continue
    if t.get("study") != 2 or t.get("model") != "gemini": continue
    key = (t["task"], t["condition"])
    counts[key] = counts.get(key, 0) + 1

print("Gemini Study 2 — gap fill plan:")
total_needed = 0
for task in TASKS:
    for cond in CONDITIONS:
        have = counts.get((task, cond), 0)
        need = max(0, TARGET - have)
        status = "OK" if need == 0 else f"need {need}"
        print(f"  {task} [{cond:8}]: {have}/{TARGET}  {status}")
        total_needed += need
print(f"\nTotal runs needed: {total_needed}\n")
...
for task in TASKS:
    ...
    for cond in CONDITIONS:
        ...
        for i in range(need):
            ...
            print(f"[gemini] {task} [{cond}] rep {have+i+1}/{TARGET} -> {run_id}")
            restore_task_sandbox(task)
            sandbox_tools.set_sandbox_dir(task_sandbox)
            try:
                transcript = run_task(task_text, task, "gemini", cond, run_id)
